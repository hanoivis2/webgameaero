...
function Physics(ui) {
  ...
  function tick(dt) {
    ...
    if (shipBody) {
      ...
      if (ui.activeKeys.left && !ui.activeKeys.right) {
        shipBody.applyAngularImpulse(0.05, true);
      ...
      if (ui.activeKeys.up) {
        var f = shipBody.getWorldVector(Vec2(0.0, 2.0));
      ...
      if (ui.activeKeys.fire && globalTime > allowFireTime) {
        ...
        var magnitude = 2, angle = shipBody.getAngle();
        ...
        bulletBody.createFixture(pl.Polygon([
          rotateVector(Vec2(-0.05, -0.2), angle),
          rotateVector(Vec2(0.05, -0.2), angle),
          rotateVector(Vec2(0, 0.2), angle)
          // Vec2(-0.05, -0.2),
          // Vec2(0.05, -0.2),
          // Vec2(0, 0.2)
        ]), {
  ...
  function rotateVector(out,angle){
